...
class MP3Player(Frame):
    ...
    def initUI(self):
        ...
        self.test           = 0
        self.counter5       = 0
        if os.path.exists ('/sys/devices/platform/i2c@0'): # check for HyperPixel4 LCD and if so disable GPIO.
            self.gpio_enable = 0
        else:
            self.gpio_enable = 1
            import RPi.GPIO as GPIO
    ...
    def Show_Track(self):
        if len(self.tunes) > 0:
            ...
            if os.path.exists(self.track):
                ...
                #else:
                #    self.Disp_Drive.config(text = "RELOADING")
                #    self.RELOAD_List()
            self.Check_Sleep()
    ...
    def playing(self):
        ...
        if os.path.exists(self.track):
            if time.time() - self.start > (self.track_len - self.gapless) - 1 and self.play == 1 and self.paused == 0:
                self.play = 0
                self.s.configure("LabeledProgressbar", text="0 %      ", background='red')
                self.progress['value'] = 0
