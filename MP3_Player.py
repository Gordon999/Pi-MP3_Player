...
class MP3Player(Frame):
    ...
    def initUI(self):
        ...
        self.paused         = 0
        self.pstart         = 0
        self.pst            = 2 # Pre-Start time (secs)
        self.count1         = 0
        self.count2         = 0
        self.version        = 2
        self.album_time     = 0
        ...
        self.Button_Pstart = tk.Button(self.master, text = "Pre-Start OFF", fg = "white", width = 7, height = 2,command=self.Pstart, wraplength=80, justify=CENTER)
    ...
    def Start_Play2(self):
        ...
        if self.album_start == 0:
            ...
            if counter > len(self.tunes) - 1:
                stop = 1
            while (self.tunes[counter].split('^')[1]) == self.album_name and stop == 0:
                self.artist_name1 = (self.tunes[counter].split('^')[0])
                self.album_name1  = (self.tunes[counter].split('^')[1])
                self.track_name1  = (self.tunes[counter].split('^')[2])
                self.drive_name1  = (self.tunes[counter].split('^')[3])
            ...
            if self.album_track == 1:
                self.Disp_Total_tunes.config(text = counter - self.track_no)
    ...
    def playing(self):
        ...
        if len(self.track_name)> 50:
            self.count2 +=1
            if self.count2 > len(self.track_name) - 50:
                self.count2 = 0
                self.Disp_track_name.config(text =self.track_name)
            if self.count2 > 5:
                self.Disp_track_name.config(text =self.track_name[self.count2 - 5:])
                
        if self.minutes == 0 and self.seconds == 0 and self.album_start > 0:
            self.play = 2
            self.album_start = 0
            self.album_time = 0 
        ...
        if os.path.exists(self.track):
            if time.time() - self.start > self.track_len - self.pstart and self.play == 1 and self.paused == 0:
                self.play = 0
                self.track_no +=1
    ...
    def Pause(self):
        ...
        if self.version == 1 and self.play == 0:
            self.version = 2
            self.pstart = 0
            self.Button_Pstart.config(fg = "white",bg = "light gray", text ="Pre-Start OFF")
    ...
    def Pstart(self):
        if self.version == 2 and self.play == 1 and self.paused == 0:
            ...
            self.version = 1
            self.Start_Play()
        if self.pstart == 0 and self.paused == 0:
            self.pstart = self.pst
            self.paused = 0
            self.version = 1
            self.Button_Pause.config(fg = "white",bg = "light gray", text ="Pause")
            self.Button_Pstart.config(fg = "black",bg = "orange", text ="Pre-Start ON")
        elif self.pstart == self.pst and self.paused == 0:
            self.pstart = 0
            self.Button_Pstart.config(fg = "black",bg = "light gray", text ="Pre-Start OFF")
    ...
    def Play_Album(self):
        if self.paused == 0 and len(self.tunes) > 0:
            if self.album_start > 0:
                ...
                self.Button_TAlbum.config(fg = "black")
                self.Button_TAlbum.config(bg = "orange")
                self.Play()
            self.Button_TAlbum.config(fg = "red", text = "Play Album & Shutdown")
            self.album_track = 0
            self.album_start = 1
            self.Start_Play()
    ...
    def Prev_m3u(self):
        if self.paused == 0:
            if self.play == 1:
                ...
                self.Button_Stop.config(bg = "light gray")
            self.album_time = 0
            self.Button_TAlbum.config(bg = "light gray", fg = "blue")
            self.shutdown = 0        
            self.play = 0
            self.m3u_no -=1
            if self.m3u_no < 0:
