...
class MP3Player(Frame):
    ...
    def playing(self):
        ...
        if len(self.album_name) > self.length:
            self.album_name += self.album_name[0]
            self.album_name = self.album_name[1:len(self.album_name)]
            self.Disp_album_name.config(text = self.album_name)
        # scroll track name
        if len(self.track_name) > self.length:
            self.track_name += self.track_name[0]
            self.track_name = self.track_name[1:len(self.track_name)]
