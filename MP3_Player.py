...
class MP3Player(Frame):
    ...
    def initUI(self):
        # Radio Stations List, add to them by adding a name & URL
        self.Radio_Stns = ["BBC Radio1", "http://a.files.bbci.co.uk/media/live/manifesto/audio/simulcast/hls/uk/sbr_high/ak/bbc_radio_one.m3u8",
                           "BBC Radio2", "http://a.files.bbci.co.uk/media/live/manifesto/audio/simulcast/hls/uk/sbr_high/ak/bbc_radio_two.m3u8",
                           "BBC Radio3", "http://a.files.bbci.co.uk/media/live/manifesto/audio/simulcast/hls/uk/sbr_high/ak/bbc_radio_three.m3u8",
                           "BBC Radio4", "http://a.files.bbci.co.uk/media/live/manifesto/audio/simulcast/hls/uk/sbr_high/ak/bbc_radio_fourfm.m3u8",
                           "BBC 6Music","http://a.files.bbci.co.uk/media/live/manifesto/audio/simulcast/hls/uk/sbr_high/ak/bbc_6music.m3u8",
                           "BBC R2","http://bbcmedia.ic.llnwd.net/stream/bbcmedia_radio2_mf_p",
                           "BBC R4FM","http://bbcmedia.ic.llnwd.net/stream/bbcmedia_radio4fm_mf_p",
                           "BBC R4LW","http://bbcmedia.ic.llnwd.net/stream/bbcmedia_radio4lw_mf_p",
                           "BBC R4Extra","http://bbcmedia.ic.llnwd.net/stream/bbcmedia_radio4extra_mf_p",
                           "BBC World Service","http://bbcwssc.ic.llnwd.net/stream/bbcwssc_mp1_ws-eieuk_backup",
                           "Classic FM","http://media-ice.musicradio.com:80/ClassicFMMP3",
                           "Cumberland Radio", "http://streams.radiomast.io/ebca0c3d-4178-4386-88a1-4db6b7d9e291",
                           "Magic Radio", "http://mp3.magic-radio.net/flac",
                           "Radio Paradise", "http://stream.radioparadise.com/rock-flac",
                           "TME Folk", "https://streams.radiomast.io/tmefolk"]
        
        self.mp3_search     = "/media/pi/*/*/*/*.mp3"   # search criteria for mp3s  (/media/pi/USBDrive Name/Artist Name/Album Name/Tracks.mp3)
        self.flac_search    = "/media/pi/*/*/*/*.flac"  # search criteria for flacs (/media/pi/USBDrive Name/Artist Name/Album Name/Tracks.flac)
        ...
        # check Lasttrack.txt exists, if not then write default values. Used for recalling last Radio Station, and restarting if using a Pi Zero.
        track = 0
        start = 0
        radio = 0
        if not os.path.exists('Lasttrack.txt'):
            with open('Lasttrack.txt', 'w') as f:
                f.write(str(track) + "\n" + str(start) + "\n" + str(radio) + "\n")

        # read Lasttrack.txt
        with open("Lasttrack.txt", "r") as file:
           track = int(file.readline())
           start = int(file.readline())
           radio = int(file.readline())
        if start == 1:
            self.track_no = track
        self.Radio        = radio
        if self.Radio > int(len(self.Radio_Stns)/2):
            self.Radio    = 0
        self.start        = start
        ...
        if self.cutdown == 0: # Pi 7" Display 800 x 480
            ...
            Button_Volume_Dn =  tk.Button(self.Frame10, text = " < Vol ",    bg = "yellow",width = 7, height = 2,command = self.volume_DN)
            ...
            self.Button_Vol_UP =  tk.Button(self.Frame10, text = "Vol >",      bg = "yellow",width = 7, height = 2,command = self.volume_UP)
            ...
            self.Button_Radio = tk.Button(self.Frame10, text = "Radio",    bg = "light blue",width = 7, height = 1,command = self.RadioX,repeatdelay=1000, repeatinterval=500)
            self.Button_Radio.grid(row =5, column = 7)
            ...
            self.Button_QUIT = tk.Button(self.Frame10, text = "QUIT",bg = "light gray",  width = 3, height = 1,command=self.exit).grid(row =9, column = 5, columnspan = 2)
            self.Button_Shutdown = tk.Button(self.Frame10, text = "Shutdown",   bg = "gray",width = 7, height = 2,command = self.Shutdown).grid(row = 9, column = 7, padx = 8)
        ...
        if self.cutdown == 1: # 320 x 240
            ...
            self.Button_Vol_DN =  tk.Button(self.Frame10, text = " < Vol " + "...", wraplength=40,    bg = "yellow",width = 4, height = 2,command = self.volume_DN)
            self.Button_Vol_DN.grid(row = 0, column = 2)
            self.Button_Vol_UP =  tk.Button(self.Frame10, text = "Vol > " + str(self.volume), wraplength=55,bg = "yellow",width = 4, height = 2,command = self.volume_UP)
            ...
            self.Button_Shutdown = tk.Button(self.Frame10, text = "Shutdn",   bg = "gray",width = 4, height = 1,command = self.Shutdown).grid(row = 6, column = 4)
        ...
        if self.cutdown == 2: # 640 x 480
            ...
            self.Button_TAlbum.grid(row = 0, column = 1,pady = 10)
            self.Button_Vol_DN =  tk.Button(self.Frame10, text = " < Vol ",    bg = "yellow",width = 5, height = 2,command = self.volume_DN)
            ...
            self.Button_Vol_UP =  tk.Button(self.Frame10, text = "Vol >",      bg = "yellow",width = 5, height = 2,command = self.volume_UP)
            self.Button_Vol_UP.grid(row = 0, column = 7)
            ...
            self.Button_QUIT = tk.Button(self.Frame10, text = "QUIT",bg = "light blue",  width = 5, height = 2,command=self.exit).grid(row = 9, column = 6)
            self.Button_Shutdown = tk.Button(self.Frame10, text = "Shutdown",   bg = "gray",width = 5, height = 2,command = self.Shutdown).grid(row = 9, column = 7)
            ...
            self.Button_Radio = tk.Button(self.Frame10, text = "Radio",    bg = "light blue",width = 5, height = 1,command = self.RadioX,repeatdelay=1000, repeatinterval=500)
            self.Button_Radio.grid(row = 6, column = 5)
        ...
        if self.cutdown == 3: # 480 x 800
            ...
            self.Button_Start.grid(row = 0, column = 0)
            self.Button_Radio = tk.Button(self.Frame10, text = "Radio",    bg = "light blue",width = 7, height = 2,command = self.RadioX,repeatdelay=1000, repeatinterval=500)
            self.Button_Radio.grid(row = 0, column = 1)
            ...
            self.Button_Vol_DN =  tk.Button(self.Frame10, text = " < Vol " + "...", wraplength=40,    bg = "yellow",width = 7, height = 2,command = self.volume_DN)
            self.Button_Vol_DN.grid(row = 0, column = 3)
            self.Button_Vol_UP =  tk.Button(self.Frame10, text = "Vol > " + str(self.volume), wraplength=55,bg = "yellow",width = 7, height = 2,command = self.volume_UP)
            ...
            self.Button_Reload.grid(row = 6, column = 0)
            self.Button_QUIT = tk.Button(self.Frame10, text = "QUIT",bg = "light blue",  width = 5, height = 2,command=self.exit).grid(row = 15, column = 3)
            self.Button_Shutdown = tk.Button(self.Frame10, text = "Shutdn",   bg = "gray",width = 5, height = 2,command = self.Shutdown).grid(row = 15, column = 4)
        ...
        if self.cutdown == 4: # 480 x 320
            ...
            self.Button_Vol_DN =  tk.Button(self.Frame10, text = " < Vol " + "...", wraplength=40,    bg = "yellow",width = 6, height = 2,command = self.volume_DN)
            self.Button_Vol_DN.grid(row = 0, column = 3)
            self.Button_Vol_UP =  tk.Button(self.Frame10, text = "Vol > " + str(self.volume), wraplength=55,bg = "yellow",width = 6, height = 2,command = self.volume_UP)
            self.Button_Vol_UP.grid(row = 0, column = 4)
            ...
            self.Button_Reload = tk.Button(self.Frame10, text = "RELOAD",width = 6, height = 2, bg = "#c5c",command = self.RELOAD_List, wraplength=80, justify=CENTER)
            self.Button_Reload.grid(row = 6, column = 0)
            self.Button_QUIT = tk.Button(self.Frame10, text = "QUIT",bg = "light blue",  width = 5, height = 2,command=self.exit).grid(row = 6, column = 4)
            self.Button_Shutdown = tk.Button(self.Frame10, text = "Shutdn",   bg = "gray",width = 5, height = 2,command = self.Shutdown).grid(row = 6, column = 5)
            self.Button_Shuffle = tk.Button(self.Frame10, text = "Shuffle", bg = "light blue",width = 6, height = 2,command = self.Shuffle_Tracks, wraplength=80, justify=CENTER)
            self.Button_Shuffle.grid(row = 6, column = 1)
            ...
            self.Button_Radio = tk.Button(self.Frame10, text = "Radio",    bg = "light blue",width = 5, height = 1,command = self.RadioX,repeatdelay=1000, repeatinterval=500)
            self.Button_Radio.grid(row = 1, column = 5)
            self.Button_Sleep = tk.Button(self.Frame10, text = "SLEEP", bg = "light blue",width = 5, height = 2,command = self.sleep,repeatdelay=1000, repeatinterval=500)
        ...
        if self.cutdown == 5: # Pi 7" Display 800 x 480 (Simple layout)
            ...
            self.Button_Vol_DN =  tk.Button(self.Frame10, text = " < Vol ", wraplength=50,    bg = "yellow", font = 50,width = 12, height = 4,command = self.volume_DN)
            self.Button_Vol_DN.grid(row = 0, column = 3)
            self.Button_Vol_UP =  tk.Button(self.Frame10, text = "Vol > " + str(self.volume), wraplength=60,bg = "yellow", font = 50,width = 12, height = 4,command = self.volume_UP)
            self.Button_Vol_UP.grid(row = 0, column = 4)
            ...
            self.Button_Next_AZ.grid(row = 5, column = 4, pady = 0)
            self.Button_Reload = tk.Button(self.Frame10, text = "RELOAD", font = 50,width = 10, height = 3,command = self.RELOAD_List, wraplength=80, justify=CENTER)
            self.Button_Reload.grid(row = 6, column = 0)
            self.Button_Radio = tk.Button(self.Frame10, text = "Radio",    bg = "light blue",width = 12, height = 4,command = self.RadioX,repeatdelay=1000, repeatinterval=500)
            self.Button_Radio.grid(row = 6, column = 3)
            self.Button_Shutdown = tk.Button(self.Frame10, text = "Shutdn", font = 50,   bg = "gray",width = 12, height = 3,command = self.Shutdown).grid(row = 6, column = 4)
    ...
    def Play(self):
        ...
        if self.Radio_ON == 1 and self.stopstart == 0 and self.album_start == 0:
            ...
            self.play = 0
            if self.cutdown != 5:
                self.Button_Prev_PList.config(text = "< P List")
                self.Button_Next_PList.config(text = "P List >")
                if self.cutdown != 1:
            ...
            if self.cutdown == 0:
                self.Button_Search_to_m3u.config(bg = "light green", fg = "black")
            if self.cutdown != 4 and self.cutdown != 5 and self.cutdown != 1:
                self.Button_Add_to_FAV.config(bg = "light green", fg = "black")
    ...
    def Prev_m3u(self):
        if self.Radio_ON == 1:
            ...
            with open('Lasttrack.txt', 'w') as f:
                f.write(str(self.track_no) + "\n" + "0" + "\n" + str(self.Radio) + "\n")
            if os.path.exists("/home/pi/Documents/" + self.Name + ".jpg") and self.cutdown !=4 and self.cutdown !=1:
                self.load = Image.open("/home/pi/Documents/" + self.Name + ".jpg")
                self.load = self.load.resize((218, 218), Image.ANTIALIAS) 
                self.render2 = ImageTk.PhotoImage(self.load)
                self.img.config(image = self.render2)
            elif os.path.exists(self.mp3_jpg) and self.cutdown !=4 and self.cutdown !=1:
                self.img.config(image = self.render)
    ...
    def Prev_Artist(self):
        if self.Radio_ON == 1 and self.cutdown == 5:
            self.Radio -= 2
            if self.Radio < 0:
                self.Radio = len(self.Radio_Stns) - 2
            self.q.kill()
            ...
            if os.path.exists("/home/pi/Documents/" + self.Name + ".jpg") and self.cutdown !=4 and self.cutdown !=5:
                self.load = Image.open("/home/pi/Documents/" + self.Name + ".jpg")
                self.load = self.load.resize((218, 218), Image.ANTIALIAS) 
                self.render2 = ImageTk.PhotoImage(self.load)
                self.img.config(image = self.render2)
            elif os.path.exists(self.mp3_jpg) and self.cutdown !=4 and self.cutdown !=5:
                self.img.config(image = self.render)
            with open('Lasttrack.txt', 'w') as f:
                f.write(str(self.track_no) + "\n" + "0" + "\n" + str(self.Radio) + "\n")
        if self.paused == 0 and self.album_start == 0 and os.path.exists(self.que_dir) and self.Radio_ON == 0:
    ...
    def RadioX(self):
        if self.paused == 0 and self.album_start == 0 and self.stopstart == 0 and self.Radio_ON == 0:
            ...
            if out == True:
                ...
                self.version = 1
                self.Name = self.Radio_Stns[self.Radio]
                self.Button_Radio.config(bg = "red",fg = "black")
                ...
                if self.cutdown != 5:
                    ...
                    self.Button_Prev_Artist.config(bg  = "light grey", fg = "white")
                    self.Button_Next_Artist.config(bg  = "light grey", fg = "white")
                    self.Button_Prev_PList.config(text = " <Station")
                    self.Button_Next_PList.config(text = " Station>")
                ...
                self.Button_Next_AZ.config(bg = "light grey", fg = "white")
                if self.cutdown == 0:
                    self.Button_Search_to_m3u.config(bg = "light grey", fg = "white")
                if self.cutdown != 4 and self.cutdown !=5 and self.cutdown !=1:
                    self.Button_Add_to_FAV.config(bg = "light grey", fg = "white")
