...
# Pi_MP3_Player v15.65

#set display format
cutdown = 0 # 0 = 800x480, 1 = 320x240, 2 = 640x480, 3 = 480x800, 4 = 480x320, 5 = 800x480(SIMPLE LAYOUT, only default Playlist)

# check Lasttrack.txt exists, if not then write default values. Used for reaclling last Radio Station, and restarting if using a Pi Zero.
track = 0
start = 0
radio = 0
...
class MP3Player(Frame):
    ...
    def initUI(self):
        # Radio Stations List
        self.Radio_Stns = ["Magic Radio", "http://mp3.magic-radio.net/flac",
        ...
        if self.cutdown == 1: # 320 x 240
            ...
            #self.Button_QUIT = tk.Button(self.Frame10, text = "QUIT", width = 4, height = 1,command=self.exit).grid(row = 6, column = 3)
        ...
        if self.cutdown == 5: # Pi 7" Display 800 x 480 (Simple layout)
            ...
            self.Button_Reload.grid(row = 6, column = 0)
            #self.Button_QUIT = tk.Button(self.Frame10, text = "QUIT",  width = 10, height = 3,command=self.exit).grid(row = 6, column = 3)
    ...
    def Prev_m3u(self):
        if self.Radio_ON == 1:
            ...
            self.p.kill()
            self.p = subprocess.Popen(["mplayer", "-nocache", self.Radio_Stns[self.Radio + 1]] , shell=False)
