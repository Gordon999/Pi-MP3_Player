...
class MP3Player(Frame):
    ...
    def playing(self):
        # scroll album name
        if len(self.album_name) > self.length:
            self.count1 +=1
            if self.count1 > self.length:
                self.count1 = 0
            if self.count1 > len(self.album_name) - (self.length - 5):
                self.count1 = 0
                self.Disp_album_name.config(text =self.album_name)
            if self.count1 > 5:
               ...
               #time.sleep(0.25)
        # scroll track name
        if len(self.track_name) > self.length:
            self.count2 +=1
            if self.count2 > self.length:
                self.count2 = 0
            if self.count2 > len(self.track_name) - (self.length - 5):
                self.count2 = 0
                self.Disp_track_name.config(text =self.track_name[:-4])
            if self.count2 > 5:
