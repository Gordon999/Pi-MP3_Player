...
# Pi_MP3_Player v16.27

#set display format
cutdown = 6 # 0 = 800x480, 1 = 320x240, 2 = 640x480, 3 = 480x800, 4 = 480x320, 5 = 800x480(SIMPLE LAYOUT, only default Playlist)
...
class MP3Player(Frame):
    ...
    def initUI(self):
        ...
        self.p_seconds      = 0
        self.copy           = 0
        self.voldn          = 5  #16                  # external volume down gpio input
        self.volup          = 6  #21                  # external volume up gpio input
        self.mute           = 9  #20                  # external mute gpio input
        self.key_stop       = 11 #12                  # external STOP gpio input
        ...
        # change buttons if using cutdown = 4
    ...
    def Show_Track(self):
        ...
        if len(self.tunes) > 0:
            ...
            if self.cutdown == 0 or self.cutdown == 2 or self.cutdown == 3 or self.cutdown == 6:
                self.render2 = ""
                if self.drive_name[-1] == "*":
    ...
    def Play(self):
        ...
        if self.album_start == 0 and self.stopstart != 1 and len(self.tunes) > 0 and self.Radio_ON == 0:
            ...
            if self.cutdown == 6:
                self.Button_Radio.config( bg = "light blue", fg = "black", text = "Gapless")
            else:
            ...
            if self.cutdown == 6:
                ...
                if self.repeat == 0 and self.repeat_album == 0:
                    self.Button_Reload.config(bg = "light blue", fg = "black", text = "Repeat")
                elif self.repeat == 1 and self.repeat_album == 0:
                    self.Button_Reload.config(bg = "green", fg = "white", text = "Repeat")
                elif self.repeat == 1 and self.repeat_album == 1:
                    self.Button_Reload.config(bg = "green", fg = "white", text = "Repeat Album")
            else:
        ...
        if self.cutdown != 1 and self.cutdown != 4 and self.cutdown != 5 :
            ...
            if abs_x > 314 and abs_x < 514 and abs_y > 152 and abs_y < 369 and self.cutdown == 6 and self.Radio_ON == 1 and self.Radio_RON == 0:
                ...
                if math.sqrt((x2*x2)+ (y2*y2)) < 30 and self.stopstart == 0 and self.Radio_ON == 0:
                    self.wheel_opt +=1
                    if (self.wheel_opt > 4 and (self.cutdown == 6 or self.cutdown == 0 or self.cutdown == 3 or self.cutdown == 2)) or (self.wheel_opt > 3 and (self.cutdown != 6 and self.cutdown != 0 and self.cutdown != 3 and self.cutdown != 2)):
                        self.wheel_opt = 0
                    ...
                    if self.cutdown == 0 or self.cutdown == 3 or self.cutdown == 2 or self.cutdown == 6:
    ...
    def nextAZ(self):
        if (self.Radio_RON == 1 or self.album_start == 1) and self.cutdown == 6:
            self.PopupInfo()
    ...
    def RELOAD_List(self):
        if self.trace == 1:
            print ("RELOAD_List")
        if self.cutdown == 6 and self.paused == 0 and (self.album_start == 1 or self.stopstart == 1) and self.Radio_ON == 0:
            if self.cutdown == 6:
                if self.play == 2:
                    self.play = 0
                self.repeat_count +=1
                if self.repeat_count == 1:
                    self.repeat_track = 1
                    self.repeat = 0
                    self.Button_Reload.config(bg = "green", fg = "white", text = "Repeat Track")
                elif self.repeat_count == 2:
                    self.repeat_track = 0
                    self.repeat = 1
                    self.Button_Reload.config(bg = "green",fg = "white",text = "Repeat P-List")
                else:
                    self.repeat_count = 0
                    self.repeat = 0
                    self.repeat_track = 0
            ...
            if self.paused == 0 and self.album_start == 1 and self.repeat_album == 0:
                ...
                self.repeat = 0
                self.repeat_track = 0
                self.Button_Reload.config(bg = "light blue",fg = "black",text = "Repeat Album")
    ...
    def RadioX(self):
        ...
        self.m.setvolume(self.volume)
        os.system("amixer -D pulse sset Master " + str(self.volume) + "%")
        if self.cutdown ==6 and (self.stopstart == 1 or self.album_start == 1):
            self.Gapless()
        # STOP RECORD BUTTON
        ...
        # STOP RADIO BUTTON
            ...
            if self.cutdown == 5 or self.cutdown == 0 or self.cutdown == 6:
        ...
        # START RADIO BUTTON
            ...
            if out == True:
                ...
                with open('Lasttrack3.txt', 'w') as f:
                    f.write(str(self.track_no) + "\n" + str(self.auto_play) + "\n" + str(self.Radio) + "\n" + str(self.volume) + "\n" + str(self.auto_radio) + "\n" + str(self.auto_record) + "\n" + str(self.auto_rec_time) + "\n" + str(self.shuffle_on) + "\n" + str(self.auto_album) + "\n")
                if self.cutdown == 0 or self.cutdown == 5 or self.cutdown == 6:
                    self.L1.config(text = "RAM: ")
                out = self.isConnected()
                rems = glob.glob("/run/shm/music/*/*/*/*/*.mp3")
                for x in range(0,len(rems)):
