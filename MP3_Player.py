...
class MP3Player(Frame):
    ...
    def playing(self):
        if len(self.album_name)> 50:
            ...
            if self.count1 > len(self.album_name) - 50:
                self.count1 = 0
            self.Disp_album_name.config(text =self.album_name[self.count1:])
        if len(self.track_name)> 50:
            self.count2 +=1
            if self.count2 > len(self.track_name) - 50:
                self.count2 = 0
            self.Disp_track_name.config(text =self.track_name[self.count2:])
    ...
    def Pstart(self):
        if self.version == 2 and self.play == 1 and self.paused == 0:
            ...
            self.version = 1
            self.Start_Play()
        if self.pstart == 0 and self.paused == 0 and self.paused == 0:
        ...
        elif self.pstart == 2 and self.paused == 0:
            self.pstart = 0
    ...
    def sleep(self):
        self.begin = time.time()
        self.sleep_current = int(self.sleep_current + 15.99)
        self.sleep_time = int(self.sleep_current/15) * 15 
