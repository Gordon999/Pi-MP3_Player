...
class Player(Frame):
    ...
    def initUI(self): 
        ...
        self.B2 =  tk.Button(self.master, text = "Stop  Play",fg = "white",width = 7, height = 2,command = self.Stop_Play)
        ...
        self.B15 = tk.Button(self.master, text = "SLEEP", bg = "light blue",width = 7, height = 2,command = self.sleep) #.grid(row = 8, column = 1,padx = 30)
        self.B15.grid(row = 8, column = 1, padx = 30)
    ...
    def Start_Play(self):
        if len(self.tunes) > 0 and self.play == 0:
            ...
            if os.path.exists(self.track):
                ...
                self.B1.config(bg = "light gray",fg = "red",text = "Playing...")
        ...
        if os.path.exists(self.track):
            ...
            if self.play == 1:
                if self.flash == 0:
                    ...
                    self.B1.config(bg = "light gray",fg = "white",text = "Playing...")
                    self.flash = 0
    ...
    def Stop_Play(self):
        if self.play == 1:
           ...
           os.killpg(self.p.pid, signal.SIGTERM)
           self.track_no -=1
           self.B1.config(bg = "green",fg = "white",text = "Start Play")
           self.B2.config(bg = "light gray")
        self.Show_Track()
    ...
    def sleep(self):
        ...
        if self.sleep_time == 0:
            self.B15.config(bg = "light blue")
            self.Disp_sleep.config(text ="OFF")
        else:
            self.B15.config(bg = "orange")
