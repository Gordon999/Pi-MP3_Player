...
fullscreen = 1
global cutdown
cutdown = 1 # 0 = 840x480, 1 = 320x240

# Pi_MP3_Player v15.10
...
class MP3Player(Frame):
    ...
    def initUI(self):
        ...
        if os.path.exists(self.mp3_jpg) and self.cutdown == 0:
    ...
    def Stop_Play(self):
        ...
        if self.play == 1:
            ...
            if self.version == 1:
                ...
                player.stop()
                self.paused = 0
                if self.BT == 0 and self.cutdown == 0:
                    self.Button_Pause.config(fg = "black",bg = "light blue", text ="Pause")
            self.Button_Start.config(bg = "green",fg = "white",text = "PLAY Playlist")
            self.Button_Stop.config(bg = "light gray")
        if self.cutdown == 0:
    ...
    def Check_switches(self):
        ...
        if (self.album_start == 0 or self.stopstart == 1) and self.cutdown == 0:
            x = self.master.winfo_pointerx()
            y = self.master.winfo_pointery()
            abs_x = self.master.winfo_pointerx() - self.master.winfo_rootx()
            abs_y = self.master.winfo_pointery() - self.master.winfo_rooty()
    ...
    def Prev_Track(self):
        if self.album_track != 1 and os.path.exists(self.que_dir):
            ...
            if self.paused == 0:
                ...
                if self.play == 2:
                    self.play = 0
                if self.version == 2 and self.cutdown == 0:
                    self.Button_Pause.config(fg = "black",bg = "light blue", text ="Pause")
                if self.play == 1:
                   if self.version == 1:
                       os.killpg(self.p.pid, signal.SIGTERM)
