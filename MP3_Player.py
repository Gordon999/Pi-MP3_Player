...
class MP3Player(Frame):
    ...
    def initUI(self):
        ...
        self.record         = 1                         # show Record button, if applicable. 
    ...
    def RadioX(self):
        ...
        if self.Radio_ON == 1 and self.Radio_RON == 1 and self.record == 1:
            ...
            if out == True:
                ...
                if self.cutdown != 5 and self.cutdown != 2 and self.cutdown != 1:
                    self.Button_PList_m3u.config(bg  = "light grey", fg = "white")
                if self.cutdown != 4 and self.cutdown !=5 and self.cutdown !=1:
    ...
    def Check_Record(self):
        ...
        if self.Radio_RON == 1 and ((time.time() - self.rec_begin > self.record_time_min)  or freeram < self.ram_min) and self.record == 1:
            if self.trace == 1:
    ...
    def Get_track(self,x):
        ...
        if len(track) > 0:
            ...
            self.Disp_track_name.config(text = track_name[counter][:-4])
        else:
    ...
    def Shutdown(self):
        ...
        USB_Files  = []
        USB_Files  = (os.listdir("/media/pi"))
        if self.Radio_ON == 1 and self.Radio_RON == 0 and self.Radio_Stns[self.Radio + 2] == 1 and self.record == 1: # and len(USB_Files) > 0:
            self.Name = ""
            ...
            self.r = subprocess.Popen(["streamripper", self.Radio_Stns[self.Radio + 1],"-l","9999","-d", "/run/shm/music/" + self.Radio_Stns[self.Radio] + "/Radio_Recordings","-a",self.Name], shell=False)
            self.Radio_RON = 1
            ...
            if self.total_record > 9900:
                self.record_time -= int((self.total_record - 9900)/60)
                self.total_record = 9900
            ...
            if self.shuffle_on == 1 and self.sleep_time > 0 and self.play == 0:
                ...
                print ("Shutdown")
                os.system("sudo shutdown -h now")
        elif len(USB_Files) == 0:
            messagebox.showinfo("WARNING!","No USB found!")
