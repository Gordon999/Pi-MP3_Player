...
class MP3Player(Frame):
    ...
    def initUI(self):
        ...
        self.HP4_backlight  = 0                         # Hyperpixeself.L4 backlight control, set to 1.    Uses GPIO 19. EXPERIMENTAL !!
    ...
    def DelPL_m3u(self):
        ...
        if self.Radio_ON == 1 and self.Radio_RON == 0:
            ...
            for x in range(0,len(rems)):
                os.remove(rems[x])
            if os.path.exists('/run/shm/music'):
                shutil.rmtree('/run/shm/music')
    ...
    def Get_track(self,x):
        ...
        if len(track) > 0 and self.Radio_ON == 1:
            ...
            if self.track_name[self.counter] != " - .mp3":
                self.tname = self.track_name[self.counter][:-4]
                self.Disp_track_name.config(text = self.tname)
            else:
                self.tname = "Unknown"
                self.Disp_track_name.config(text = self.tname)
