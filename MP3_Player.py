...
class MP3Player(Frame):
    ...
    def initUI(self):
        ...
        self.dsf_search     = "/media/pi/*/*/*/*.dsf"   # search criteria for DSFs  (/media/pi/USBDrive Name/Artist Name/Album Name/Tracks.dsf)
        self.m4a_search     = "/media/pi/*/*/*/*.M4A"   # search criteria for M4As  (/media/pi/USBDrive Name/Artist Name/Album Name/Tracks.M4A)
        ...
        self.copy           = 0
        self.trackdata      = []
        self.voldn          = 5  # external volume down gpio input
        self.volup          = 6  # external volume up gpio input
        self.mute           = 9  # external mute gpio input
        self.key_stop       = 11 # external STOP gpio input
        self.ac             = 0
        self.bc             = 0
        self.cc             = 0
        self.old_artists    = []
        ...
        #read USB drives
        self.USB_Drives = os.listdir("/media/pi")
        ...
        if self.cutdown == 7: # Pi 7" Display 800 x 480 with scrollbars
            ...
            self.Button_Next_PList =  tk.Button(self.Frame10, text = "P-list >",   bg = "light blue",width = 7, height = 1,command = self.Next_m3u,repeatdelay=1000, repeatinterval=500)
            self.Button_Next_PList.grid(row = 1, column = 7)
            self.Button_Prev_Artist =  tk.Button(self.Frame10, text = "< Artist",   bg = "light blue",width = 7, height = 2,command = self.Prev_Artist,repeatdelay=1000, repeatinterval=500)
            self.Button_Prev_Artist.grid(row = 2, column = 0)
            self.Button_Next_Artist =  tk.Button(self.Frame10, text = "Artist >",   bg = "light blue",width = 7, height = 2,command = self.Next_Artist,repeatdelay=1000, repeatinterval=500)
            self.Button_Next_Artist.grid(row = 2, column = 7)
            self.Button_Prev_Album =  tk.Button(self.Frame10, text = "< Album",    bg = "light blue",width = 7, height = 2,command = self.Prev_Album,repeatdelay=1000, repeatinterval=500)
            ...
            if os.path.exists(self.mp3_jpg):
                ...
                self.img = tk.Label(self.Frame10, image = self.render)
                self.img.grid(row = 5, column = 0, columnspan = 2, rowspan = 5, pady = 2)
            else:
                self.img = tk.Label(self.Frame10)
                self.img.grid(row = 5, column = 0, columnspan = 2, rowspan = 5, pady = 2)
        ...
        if not os.path.exists(self.que_dir):
            ...
            for counter in range (0,len(self.Tracks)):
                counter2 = self.Tracks[counter].count('/')
                if counter2 == 6:
                    z,self.drive_name1,self.drive_name2,self.drive_name,self.artist_name,self.album_name,self.track_name  = self.Tracks[counter].split('/')
                ...
                if counter2 == 5:
                    self.drive_name1,self.drive_name2,self.drive_name,self.artist_name3,self.album_name3,self.track_name  = self.Tracks[counter].split('/')
    ...
    def album_callback(self,a):
        ...
        if self.shuffle_on == 0 or self.album_start == 1:
          if self.next_album == 0:
            ...
            for x in range(0,len(self.tunes)):
                self.artist_name_1,self.album_name_1,self.track_name_1,self.drive_name_1,self.drive_name1_1,self.drive_name2_1  = self.tunes[x].split('^')
                if self.artist_name_1 == self.new_artist and self.album_name_1[:-1] == self.new_album[:-1]:
                    self.trackdata.append(self.track_name_1)
            self.trackdata = list(dict.fromkeys(self.trackdata))
            self.Disp_track_name["values"] = self.trackdata
            if self.ac == 0 and len(self.trackdata) > 0:
        ...
        self.render2 = "" ####
        if self.drive_name[-1] == "*":
            jpath = os.path.join("/" + self.drive_name1,self.drive_name2,self.drive_name[:-1], self.artist_name + " - " + self.album_name, "*.jpg")
        else:
            jpath = os.path.join("/" + self.drive_name1,self.drive_name2,self.drive_name, self.artist_name, self.album_name, "*.jpg")
        if self.cutdown == 0 or self.cutdown == 7 or self.cutdown == 2 or self.cutdown == 3 or self.cutdown == 6:
            pictures = glob.glob(jpath)
            #print (jpath,pictures)
            self.render2 = ""
            if len(pictures) > 0:
                if len(pictures) > 1:
            ...
            elif self.gapless == 0 and self.track_type == 0:
                if os.path.exists(self.mp3c_jpg):
        ...
        if os.path.exists(self.track):
                if self.track[-4:] == ".mp3" or self.track[-4:] == ".MP3":
    ...
    def Show_Track(self):
        ...
        if len(self.tunes) > 0:
            ...
            if self.cutdown != 7:
                ...
                if self.track[-4:] == ".mp3" or self.track[-4:] == ".MP3" or self.track[-4:] == ".dsf" or self.track[-4:] == ".M4A":
            ...
            if self.cutdown == 0 or self.cutdown == 7 or self.cutdown == 2 or self.cutdown == 3 or self.cutdown == 6:
                ...
                if len(pictures) > 0: 
                    ...
                    if self.timer4 == 0:
                        self.img.config(image = self.render2)
                elif self.version == 2 and self.cutdown != 7: 
                    if os.path.exists(self.mp3c_jpg):
                        self.img.config(image = self.renderc)
                else:
                    if os.path.exists(self.mp3_jpg):
            ...
            if os.path.exists(self.track):
                if self.track[-4:] == ".mp3" or self.track[-4:] == ".MP3" :
                    ...
                    self.track_len = 60
                if self.track[-4:] == ".dsf" or self.track[-4:] == ".M4A":
                    self.track_type = 1
                ...
                if self.cutdown < 6:
                    if self.track[-4:] == "flac" and len(audio) > 0:
                        try:
                            ...
                            pass
                    elif self.track[-4:] == ".mp3" or self.track[-4:] == ".MP3" and len(audio) > 0:
                        try:
    ...
    def Play(self):
        ...
        if self.album_start == 0 and self.stopstart != 1 and len(self.tunes) > 0 and self.Radio_ON == 0:
            ...
            if self.version == 2 and self.track_type == 0:
    ...
    def Wheel_Opt_Button(self,u):
        if self.cutdown != 1 and self.cutdown != 4 and self.cutdown != 5 :
            if os.path.exists(self.mp3_jpg):
                if self.gapless == 0 and self.paused == 0 and self.track_type == 0:
            ...
            if abs_x > 314 and abs_x < 514 and abs_y > 152 and abs_y < 369 and self.cutdown == 6 and self.Radio_ON == 1 and self.Radio_RON == 0:
                ...
                if math.sqrt((x2*x2)+ (y2*y2)) < 30 and self.stopstart == 0 and self.Radio_ON == 0:
                    ...
                    if self.wheel_opt == 0 and self.cutdown != 7:
                        self.Button_Prev_Artist.config(fg = "red")
                    if self.wheel_opt == 1 and self.cutdown != 7:
                        self.Button_Prev_Album.config(fg = "red")
                    if self.wheel_opt == 2 and self.cutdown != 7:
                        self.Button_Prev_Track.config(fg = "red")
                    if self.wheel_opt == 3  and self.cutdown != 6 and self.cutdown != 7:
          ...
          if os.path.exists(self.track):  
            ...
            if os.path.exists(self.track):
                ...
                if self.version == 2:
                    ...
                    self.p = subprocess.Popen(["mplayer","-novideo",self.track], shell=False)
                if self.track[-4:] == ".mp3" or self.track[-4:] == ".MP3":       
                    audio = MP3(self.track)
                    self.track_len = audio.info.length
                    self.track_type = 0
    ...
    def Start_Play2(self):
        ...
        if self.album_start == 0:
            ...
            if self.trace == 1:
                print ("Start_Play2 - self.album_start = 1",self.track_no)
            stop = 0
            self.album_time = 0
            self.album_track += 1
            counter = self.track_no + 1
    ...
    def playing(self):
        ...
        # backlight off
        if time.monotonic() - self.light_on > self.light and (self.HP4_backlight == 1 or self.WS28_backlight == 1):
        ...
        if os.path.exists(self.track):
            ...
            if (time.monotonic() - self.start > (self.track_len - self.gapless) - (self.BT) and self.play == 1 and self.paused == 0) or (self.version == 1 and poll != None and self.play == 1 and self.paused == 0):
                if self.trace == 1:
    ...
    def Pause(self):
        ...
        if self.Radio_ON == 1 and self.Radio_RON == 0 and self.Radio_Stns[self.Radio + 2] == 1 and self.record == 1:
            ...
            self.Disp_album_name.config(text = "Radio_Recordings")
        ...
        #=======================================================================================================================
        # PAUSE BUTTON
        if self.BT == 0 and self.Radio_ON == 0 and self.track_type == 0:
            if self.trace == 1:
                print ("Pause")
            ...
            # if playing and self.paused set (paused) then unset it and Unpause
                ...
                if self.cutdown == 6:
                    self.Button_Radio.config(fg = "black",bg = "light blue", text ="Gapless")
                if os.path.exists(self.mp3c_jpg) and self.cutdown !=1 and self.cutdown != 4 and self.cutdown != 5 and self.track_type == 0:
                    self.img.config(image = self.renderc)
                player.pause()
    ...
    def Play_Album(self):
        ...
        if self.paused == 0 and len(self.tunes) > 0 and self.album_start == 0 and self.stopstart == 0 and self.Radio_ON == 0:
            ...
            if drive_name[-1] == "*":
                ...
                album = os.path.join("/" + drive_name1,drive_name2,drive_name[:-1],artist_name3,album_name3)
            else:
            ...
            if len(self.tunes) > 0:
                ...
                self.play = 0
                if self.trace == 1:
                    print ("Play Album2",self.track_no,self.tcount)
    ...
    def Stop_Play(self):
        ...
        if self.wheel_opt == 2 and self.cutdown != 7:
            self.Button_Prev_Track.config(fg = "red")
        if self.wheel_opt == 3 and self.cutdown != 7:
            self.Button_Prev_PList.config(fg = "red")
        self.Button_Next_Track.config(bg = "light blue", fg = "black")
        self.Button_Reload.config(bg = "#c5c", fg = "black", text = "RELOAD")
        if self.cutdown != 6:
    ...
    def Prev_m3u(self):
        if self.paused == 0 and self.album_start == 0 and os.path.exists(self.que_dir) and self.Radio_ON == 0:
            self.wheel_opt = 3
            if self.cutdown != 5 and self.cutdown != 6  and self.cutdown != 7:
                self.Button_Prev_PList.config(fg = "red")
            self.Button_Prev_Artist.config(fg = "black")
            self.Button_Prev_Album.config(fg = "black")
            self.Button_Prev_Track.config(fg = "black")
            ...
            if len(self.tunes) > 0:
                while (self.tunes[self.track_no].split('^')[1]) == self.album_name and stop == 0:
                    ...
                    if self.track_no > len(self.tunes) - 1:
                        ...
                        stop = 1
                if self.cutdown == 7:
                    self.new_artist,self.new_album,self.track_name,self.drive_name,self.drive_name1,self.drive_name2  = self.tunes[self.track_no + 1].split('^')
                    self.next_album = 1
                if self.play == 1:
                    if self.track_no != 0:
                        self.track_no -=1
    ...
    def RELOAD_List(self):
        ...
        if self.paused == 0 and self.album_start == 0 and self.stopstart == 0 and self.Radio_ON == 0:
            ...
            self.DSF = glob.glob(self.dsf_search)
            if len (self.DSF) > 0 :
                for j in range(0,len(self.DSF)):
                    self.Tracks.append(self.DSF[j])
            # search for dsf files
            self.m4a = glob.glob(self.m4a_search)
            if len (self.m4a) > 0 :
                for j in range(0,len(self.m4a)):
                    self.Tracks.append(self.m4a[j])
    ...
    def Roberts_mp2(self):
        # rename Roberts or Yaakin .mp2 to .mp3 and move
        USB_Files   = []
        Radio_Files = []
        New_Drive   = ""
        USB_Files   = os.listdir("/media/pi")
        #print (self.USB_Drives,USB_Files)
        for x in range (0,len(USB_Files)):
            if USB_Files[x] not in self.USB_Drives:
                New_Drive = USB_Files[x]
        if New_Drive != "":
            for x in range(0,len(USB_Files)):
                Radio_Files += glob.glob("/media/pi/" + New_Drive + "/*/*.mp2")
            for x in range(0,len(Radio_Files)):
                af,bf,cf,df,ef,ff = Radio_Files[x].split("/")
                #print(af,bf,cf,df,ef,ff)
                if ff.count(',') == 2:
                    gf,hf,jf = ff.split(",")
                    kf,lf = jf.split("-")
                    new_dir1 = "/media/pi/" + df + "/" + gf 
                    new_dir2 = "/media/pi/" + df + "/" + gf + "/" + hf + "_" + kf + "-" + lf[0:2]
                elif ff.count('_') == 1:
                    now = datetime.datetime.now()
                    tf = now.strftime("%y%m%d")
                    gf,lf = ff.split("_")
                    new_dir1 = "/media/pi/" + df + "/" + gf 
                    new_dir2 = "/media/pi/" + df + "/" + gf + "/" + tf + "-" + lf[0:-4]
                path1 = 'mkdir ' + "'" + new_dir1 + "'"
                path2 = 'mkdir ' + "'" + new_dir2 + "'"
                if not os.path.exists(new_dir1 ):
                    os.system('mkdir ' + path1)
                if not os.path.exists(new_dir2 ):
                    os.system('mkdir ' + path2)
                if ff.count(',') == 2:
                    new_track = new_dir2 + "/" + jf[0:-1] + "3"
                elif ff.count('_') == 1:
                    new_track = new_dir2 + "/" + lf[0:-1] + "3"
                if not os.path.exists(new_track):
                    shutil.copy(Radio_Files[x],new_track)
                    if os.path.exists(new_track):
                         os.remove(Radio_Files[x])
        # search for Roberts or Yaakin mp3 files
            self.mp3 = glob.glob("/media/pi/" + New_Drive + "/*/*/*.mp3")
            if len (self.mp3) > 0 :
                for j in range(0,len(self.mp3)):
                    #if self.mp3[j] not in self.tunes:
                    self.Tracks.append(self.mp3[j])
                    z,self.drive_name1,self.drive_name2,self.drive_name,self.artist_name,self.album_name,self.track_name  = self.mp3[j].split('/')
                    self.tunes.append(self.artist_name + "^" + self.album_name + "^" + self.track_name + "^" + self.drive_name + "^" + self.drive_name1 + "^" + self.drive_name2)
            # search for Roberts or Yaakin MP3 files
            self.MP3 = glob.glob("/media/pi/" + New_Drive + "/*/*/*.MP3")
            if len (self.MP3) > 0 :
    ...
    def Get_track(self,x):
        ...
        if self.Radio_ON == 1 and self.Radio_Stns[self.Radio + 2] == 1:
            if self.cutdown == 7:
                ...
                self.Disp_album_name.set("Radio_Recordings")
            else:
