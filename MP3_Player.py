...
class MP3Player(Frame):
    ...
    def initUI(self):
        self.voldn = 36       # volume down gpio input
        self.volup = 40       # volume up gpio input
        self.stop_start = 38  # start / stop play gpio input
    ...
    def RELOAD_List(self):
        if self.paused == 0 and self.album_start == 0:
            ...
            self.sorted == 0
            self.Button_AZ_artists.config(bg = "light blue",fg = "blue",text = "A-Z Artists OFF")
            Tracks = glob.glob(self.tunes_dir + "*/*/*/*.mp3")
            self.tunes = []
            if len (Tracks) > 0 :
