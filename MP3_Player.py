...
class MP3Player(Frame):
    ...
    def initUI(self):
        ...
        self.Radio_Stns = ["Magic Radio", "http://mp3.magic-radio.net/flac",
                           "Paradise Rock", "http://stream.radioparadise.com/rock-flac"
                          ]
        self.Button_info_on = 1
        ...
        if self.cutdown == 2: # 640 x 480
            ...
            self.Button_Radio = tk.Button(self.Frame10, text = "Radio",    bg = "light blue",width = 5, height = 2,command = self.RadioX, wraplength=40, justify=CENTER)
        ...
        if self.cutdown == 5: # Pi 7" Display 800 x 480 (Simple layout)
            ...
            self.Button_Reload.grid(row = 6, column = 0)
            self.Button_Radio = tk.Button(self.Frame10, text = "Radio",    bg = "light blue",width = 12, height = 4,command = self.RadioX, wraplength=80, justify=CENTER)
            self.Button_Radio.grid(row = 6, column = 3)
            ...
            if os.path.exists(self.track):
                ...
                if self.track[-4:] == "flac" and len(audio) > 0:
                    try:
                        ...
                        self.Disp_track_name.config(text = str(track_number )[2:-2] + " " + str(track_title)[2:-2])
                    except:
                        pass
                elif self.track[-4:] == ".mp3" and len(audio) > 0:
                    try:
                        track_title = audio["TIT2"].text
                        track_number = str(audio["TRCK"].text)
                        tn1,tn2 = track_number.split("/")
                        self.Disp_track_name.config(text = str(tn1)[2:] + " " + str(track_title)[2:-2])
                    except:
                        pass
    ...
    def playing(self):
        ...
        if os.path.exists(self.track):
            ...
            if time.time() - self.start > (self.track_len - self.gapless) - (4) and self.play == 1 and self.paused == 0 and (self.BT == 1 or self.track[-4:] == "flac"):
                self.Fade()
    ...
    def RadioX(self):
        if self.Radio_ON == 1 and self.stopstart == 0 and self.album_start == 0:
            ...
            self.Button_Prev_Artist.config(bg = "light blue", fg = "black", text = " < Artist")
            self.Button_Prev_Album.config(bg = "light blue", fg = "black")
            self.Button_Prev_Track.config(bg = "light blue", fg = "black")
            self.Button_Next_Artist.config(bg = "light blue", fg = "black", text = " Artist >")
            self.Button_Next_Album.config(bg = "light blue", fg = "black")
            self.Button_Next_Track.config(bg = "light blue", fg = "black")
            if self.cutdown == 0 or self.cutdown == 3 or self.cutdown == 2:
                self.Button_Next_AZ.config(bg = "light blue", fg = "black")
            if self.cutdown == 4:
                self.Button_Track_m3u.config(bg = "light blue", fg = "black")
                self.Button_Artist_m3u.config(bg = "light blue", fg = "black")
                self.Button_Album_m3u.config(bg = "light blue", fg = "black")
                self.Button_PList_m3u.config(bg = "light blue", fg = "black")
            self.Button_Reload.config(bg = "#c5c", fg = "black")
            self.Button_Next_AZ.config(bg = "light blue", fg = "black")
            self.Button_Shuffle.config(fg = "black",bg = "light blue")
            if self.cutdown != 1 and self.cutdown != 4 and self.cutdown != 5:
            ...
            if out == True:
                ...
                self.Name = self.Radio_Stns[self.Radio]
                if self.cutdown != 1:
                    self.Button_Radio.config(bg = "red",fg = "white", text = "STOP Radio")
                else:
                    self.Button_Radio.config(bg = "red",fg = "white", text = "STOP")
                if self.cutdown != 5:
                    if self.cutdown != 1:
                        self.Disp_Name_m3u.delete('1.0','20.0')
    ...
    def PopupInfo(self):
        ...
        if os.path.exists(ipath + filename):
            ...
            for txt in txts:
                ...
                if "fing" in filename or "md5" in filename or "ffp" in filename:
                    pass
                else:
                    infofile = filename
                    break
        if infofile != '':
            ...
            STxtBox = ScrolledText(popup, height=500, width=350)
    ...
    def Shutdown(self):
        if (self.WS28_backlight == 1 or self.HP4_backlight == 1) :
            self.gpio.set_PWM_dutycycle(self.pwm_pin,self.bright)
        if self.shuffle_on == 1 and self.sleep_time > 0 and self.play == 0 and self.Radio_ON == 0:
            self.exit()
        else:
            os.system("sudo shutdown -h now")
            print ("Shutdown")
