...
class MP3Player(Frame):
    ...
    def initUI(self):
        # Radio Stations List, add to them by adding a 'name','URL','recordable : 0=no 1=yes'
        self.Radio_Stns = ["Radio Paradise Rock", "http://stream.radioparadise.com/rock-flac",0,
                           "Radio Paradise Rock","http://stream.radioparadise.com/rock-192",1,
                           "Radio Paradise Main", "http://stream.radioparadise.com/flac",0,
                           "Radio Paradise Main","http://stream.radioparadise.com/mp3-320",1,
                           "JB Radio-2","http://cristina.torontocast.com:8007/mp3-320",1,
        ...
        self.radio_jpg      = "radio.jpg"               # Radio logo
        ...
        # check Lasttrack2.txt exists, if not then write default values. Used for recalling last Radio Station, and restarting if using a Pi Zero.
        track  = 0
        start  = 0
        radio  = 0
        volume = 80
        if not os.path.exists('Lasttrack2.txt'):
            with open('Lasttrack2.txt', 'w') as f:
                f.write(str(track) + "\n" + str(start) + "\n" + str(radio) + "\n" + str(volume) + "\n")

        # read Lasttrack2.txt
        with open("Lasttrack2.txt", "r") as file:
        ...
        # check for HyperPixeself.L4 LCD and if so disable GPIO controls.
        ...
        if self.cutdown == 4: # 480 x 320
            ...
            self.Button_Vol_DN =  tk.Button(self.Frame10, text = " < Vol " + "...", wraplength=40,    bg = "yellow",width = 6, height = 2,command = self.volume_DN,repeatdelay=1000, repeatinterval=500)
            self.Button_Vol_DN.grid(row = 0, column = 3)
            self.Button_Vol_UP =  tk.Button(self.Frame10, text = "Vol > " + str(self.volume), wraplength=55,bg = "yellow",width = 6, height = 2,command = self.volume_UP,repeatdelay=1000, repeatinterval=500)
            self.Button_Vol_UP.grid(row = 0, column = 4)
          ...
          if os.path.exists(self.track):  
            ...
            if self.track[-4:] == "flac" and len(audio) > 0:
                ...
                try:
                    ...
                    if tn1 < 10:
                        tn1 = "0" + tn1
                    self.Disp_track_name.config(text = str(tn1) + " " + str(track_title)[2:-2])
                except:
                    pass
    ...
    def Prev_Artist(self):
        ...
        if self.Radio_ON == 1:
            ...
            if self.Radio_Stns[self.Radio + 2] == 1 and self.record == 1:
                ...
                self.Button_Shutdown.config(fg = "black", bg = "gray", text = "Shutdown")
                if self.cutdown != 1 and self.cutdown != 4 and  self.cutdown != 5:
                    self.L8.config(text = "")
    ...
    def RadioX(self):
        ...
        if self.Radio_ON == 1 and self.Radio_RON == 1 and self.record == 1:
            ...
            if self.cutdown != 5:
                self.Disp_Total_Plist.config(text = "")
            if self.cutdown != 1 and self.cutdown != 4 and  self.cutdown != 5:
                self.Button_DELETE_m3u.config(text = "CLR RAM", bg = "light blue", fg = "black")
                self.L8.config(text = "")
                if self.cutdown != 3 and self.cutdown != 2:
                    self.L3.config(text = "")
                    self.L1.config(text = "")
            ...
            if self.cutdown != 1 and self.cutdown != 4 and  self.cutdown != 5:
                ...
                self.L6.config(text="Playlist :")
            if self.cutdown == 5 or self.cutdown == 0:
            ...
            if self.cutdown != 4 and self.cutdown != 5 and self.cutdown != 1:
                ...
                self.Button_DELETE_m3u.config(text = "DEL.m3u",bg  = "light grey", fg = "black")
            if self.cutdown != 5:
                self.Button_Prev_PList.config(fg = "black")
            ...
            if out == True:
                ...
                if self.Radio_Stns[self.Radio + 2] == 1 and self.record == 1:
                    self.Button_Shutdown.config(fg = "black", bg = "light blue", text = "RECORD")
                    self.Disp_played.config(text ="00:00")
                    if self.cutdown != 1 and self.cutdown != 4 and  self.cutdown != 5:
                        self.L8.config(text = ".mp3")
                        self.L6.config(text="")
                else:
                ...
                if self.cutdown != 1 and self.cutdown != 4:
                    if self.cutdown != 3 and self.cutdown != 2:
                        self.L1.config(text="")
                        self.L3.config(text="")
                    if self.cutdown != 2 and self.cutdown != 5:
                        self.L5.config(text="")
                    if self.cutdown != 5:
                        self.L6.config(text="")
                if self.cutdown != 1:
    ...
    def Check_Record(self):
        ...
        if self.Radio_RON == 1 and self.record == 1:
            ...
            r_minutes = int(self.rplayed // 60)
            r_seconds = int (self.rplayed - (r_minutes * 60))
            if r_seconds != self.roldsecs:
                self.Disp_played.config(text ="%02d:%02d" % (r_minutes, r_seconds % 60),fg = "red")
                self.roldsecs = r_seconds
            rec_stat = 0
            ...
            if self.record_current > 0:
                self.Button_Shutdown.config(fg = "white", bg = "red", text = str(self.record_current + 1)  + " mins")
            else:
                self.Button_Shutdown.config(fg = "yellow", bg = "red", text = str(int((self.record_time_min - (time.time() - self.rec_begin)))) + " secs")
        self.Get_track(1)

        if self.Radio_ON == 1 and self.Radio_RON == 0 and time.time() - self.rec_begin > 9900:
            self.rec_begin = time.time()
            self.q.kill()
            self.r.kill()
            time.sleep(1)
        ...
        if self.Radio_RON == 1 and ((time.time() - self.rec_begin > self.record_time_min)  or freeram < self.ram_min) and self.record == 1 and self.oldtrack != self.track_name[self.counter]:
            if self.trace == 1:
            ...
            if self.cutdown != 1:
                ...
                self.Button_Radio.config(bg = "orange",fg = "black", text = "STOP")
            self.Copy_Record()
        if self.track_name[self.counter] != " - .mp3": 
            self.oldtrack = self.track_name[self.counter]
        if self.Radio_ON == 1:
    ...
    def Get_track(self,x):
        ...
        if len(track) == 0 and self.Radio_ON == 1:
            self.Disp_track_name.config(text = "Unknown")
        if len(track) > 0 and self.Radio_ON == 1:
            self.counter = track[0].count('/')
            self.track_name = track[0].split("/",self.counter)
            if self.track_name[self.counter] != " - .mp3":
                self.Disp_track_name.config(text = self.track_name[self.counter][:-4])
            else:
                self.Disp_track_name.config(text = "Unknown")
    ...
    def Shutdown(self):
        ...
        if self.Radio_ON == 1 and self.Radio_RON == 0 and self.Radio_Stns[self.Radio + 2] == 1 and self.record == 1:
            self.Name = ""
            if self.cutdown == 0 or self.cutdown == 5:
                self.L1.config(text = "RAM: ")
            if self.cutdown != 1 and self.cutdown != 4 and  self.cutdown != 5:
                self.L8.config(text = ".mp3")
                self.Disp_Name_m3u.config(background="light gray", foreground="black")
                self.Name = str(self.Disp_Name_m3u.get('1.0','20.0')).strip()
                self.Button_DELETE_m3u.config(text = "", bg = "light grey", fg = "white")
            ...
            self.Disp_track_len.config(text ="10:00")
            self.L4.config(text="of")
            self.Button_Shutdown.config(fg = "white", bg = "red", text = str(self.record_time) + " mins")
            if self.cutdown != 1:
            ...
            if track_name != self.track_name or track_name[counter] == " - .mp3":
                rems = glob.glob("/run/shm/music/*/*/*/*/*.mp3")
                for x in range(0,len(rems)):
                    os.remove(rems[x])
            rems = glob.glob("/run/shm/music/*/*/*.cue")
            ...
            self.Disp_track_len.config(text ="%02d:%02d" % (t_minutes, t_seconds % 60))
            self.record_current = int((self.total_record - (time.time() - self.rec_begin))/60)
            self.Button_Shutdown.config(fg = "yellow", bg = "red", text = str(self.record_current + 1) + " mins")
            if self.Radio_ON == 1 and self.Radio_RON == 1 and self.shutdown == 1 and self.record_sleep == 1:
                ...
                self.sleep_time = int(self.sleep_time_min / 60)
        elif self.Radio_ON == 0 or (self.Radio_ON == 1 and self.Radio_Stns[self.Radio + 2] == 0):
