...
fullscreen = 1
global cutdown
cutdown = 4 # 0 = 800x480, 1 = 320x240, 2 = 640x480, 3 = 480x800 4 = 480x320

# Pi_MP3_Player v15.30
...
class MP3Player(Frame):
    ...
    def initUI(self):
        ...
        if len(alsaaudio.mixers()) > 0:
            for mixername in alsaaudio.mixers():
                if str(mixername) == "PCM" or str(mixername) == "Master" or str(mixername) == "Capture" or str(mixername) == "Headphone":
    ...
    def Play(self):
        if self.gpio_enable == 0 and (self.album_start == 1 or self.stopstart == 1):
            self.gpio.set_PWM_dutycycle(19,self.bright)
    ...
    def playing(self):
        if time.time() - self.light_on > self.light and self.gpio_enable == 0:
