...
class MP3Player(Frame):
    ...
    def initUI(self):
        ...
        self.stop_start     = 20                     # external start / stop play gpio input
        self.repeat         = 0
        self.play           = 0
        self.track_no       = 0
        self.sleep_time     = 0
        ...
        if len(alsaaudio.mixers()) > 0:
            for mixername in alsaaudio.mixers():
                if str(mixername) == "PCM" or str(mixername) == "Master" or str(mixername) == "Capture":
        ...
        if self.cutdown == 1:
            ...
            self.B3 =  tk.Button(self.Frame10, text = "<P-list",   bg = "light blue",width = 3, height = 1,command = self.Prev_m3u,repeatdelay=1000, repeatinterval=500)
            self.B3.grid(row = 1, column = 0)
            self.B4 =  tk.Button(self.Frame10, text = "P-list>",   bg = "light blue",width = 3, height = 1,command = self.Next_m3u,repeatdelay=1000, repeatinterval=500)
            self.B4.grid(row = 1, column = 4)
            self.B5 =  tk.Button(self.Frame10, text = "<Artist",   bg = "light blue",fg = "red",width = 3, height = 1,command = self.Prev_Artist,repeatdelay=1000, repeatinterval=500)
            self.B5.grid(row = 2, column = 0)
            self.B6 =  tk.Button(self.Frame10, text = "Artist>",   bg = "light blue",width = 3, height = 1,command = self.Next_Artist,repeatdelay=1000, repeatinterval=500)
            self.B6.grid(row = 2, column = 4)
            self.B7 =  tk.Button(self.Frame10, text = "<Album",    bg = "light blue",width = 3, height = 1,command = self.Prev_Album,repeatdelay=1000, repeatinterval=500)
            self.B7.grid(row = 3, column = 0)
            self.B8 =  tk.Button(self.Frame10, text = "Album>",     bg = "light blue",width = 3, height = 1,command = self.Next_Album,repeatdelay=1000, repeatinterval=500)
            self.B8.grid(row = 3, column = 4)
            self.B9 =  tk.Button(self.Frame10, text = "<Track",    bg = "light blue",width = 3, height = 1,command = self.Prev_Track,repeatdelay=1000, repeatinterval=500)
            self.B9.grid(row = 4, column = 0)
            self.B10 = tk.Button(self.Frame10, text = "Track>",    bg = "light blue",width = 3, height = 1,command = self.Next_Track,repeatdelay=1000, repeatinterval=500)
            self.B10.grid(row =4, column = 4)
            self.B13 = tk.Button(self.Frame10, text = "NextAZ",   width = 3, height = 1,bg = "light blue",command=self.nextAZ,repeatdelay=250, repeatinterval=500)
        ...
        if os.path.exists(self.mp3_jpg) and (self.cutdown != 1):
    ...
    def playing(self):
        # scroll artist name
        if len(self.artist_name7) > self.length:
            if self.artist_name7[0] == "_":
                self.artist_name7 += " "
            else:
        ...
        # scroll album name
        if len(self.album_name7) > self.length:
            if self.album_name7[0] == "_":
                self.album_name7 += " "
            else:
        ...
        # scroll track name
        if len(self.track_name7) > self.length:
            if self.track_name7[0] == "_":
                self.track_name7 += " "
            else:
    ...
    def Check_switches(self):
        ...
        if (self.album_start == 0 or self.stopstart == 1) and (self.cutdown == 0 or self.cutdown == 2 or self.cutdown == 3):
            x = self.master.winfo_pointerx()
            y = self.master.winfo_pointery()
            abs_x = self.master.winfo_pointerx() - self.master.winfo_rootx()
            abs_y = self.master.winfo_pointery() - self.master.winfo_rooty()
