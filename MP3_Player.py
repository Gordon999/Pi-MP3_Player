...
class MP3Player(Frame):
    ...
    def playing(self):
        ...
        if self.minutes == 0 and ((self.gapless == 0 and self.seconds == 0) or (self.gapless == self.gapless_time and self.seconds <= self.gapless_time)) and self.album_start == 1:
            ...
            self.Button_repeat.config(fg = "blue",bg = "light blue")
    ...
    def Play_Album(self):
        if self.paused == 0 and len(self.tunes) > 0 and self.album_start == 0:
            ...
            self.Button_Shuffle.config(fg = "white",bg = "light grey")
            self.Button_AZ_artists.config(fg = "white",bg = "light grey")
            self.Button_repeat.config(fg = "white",bg = "light grey")
    ...
    def Check_switches(self):
        ...
        if self.album_start == 0 or self.stopstart == 1:
            ...
            if x2 > 0 and y2 < 0:
                t = (0 - math.atan(x2/y2))
            if x2 > 0 and y2 > 0:
                t =  ((3.06 - math.atan(x2/y2)))
            if x2 < 0 and y2 > 0:
                t =  (3.06 + (0- math.atan(x2/y2)))
            if x2 < 0 and y2 < 0:
                t = (6.12 - math.atan(x2/y2))
            t = int(t * 4)
            if math.sqrt((x2*x2)+ (y2*y2)) > 40 and math.sqrt((x2*x2)+ (y2*y2)) < 100 :
                if os.path.exists(self.mp3_jpg):
                    ...
                    self.timer4 = time.time()
                self.wheel = 1
                if self.old_t > 22 and t < 2:
                    self.old_t = 0
                elif self.old_t < 4 and t > 20:
                    self.old_t = 24
                if t > self.old_t:
                    self.old_t = t
