...
class MP3Player(Frame):
    ...
    def initUI(self):
        ...
        if os.path.exists ('/sys/devices/platform/i2c@0'): 
            ...
            if self.HP4_backlight == 1:
                ...
                self.gpio.set_PWM_frequency(self.pwm_pin,6006)
            ...
            if self.cutdown == 4 and self.WS28_backlight == 1:
                os.system("sudo pigpiod")
                import pigpio
                self.gpio = pigpio.pi()
                self.pwm_pin = 18
        ...
        if self.cutdown == 0: # Pi 7" Display 800 x 480
            ...
            B12 = tk.Button(self.Frame10, text = "QUIT",bg = "light gray",  width = 7, height = 1,command=self.exit).grid(row =9, column = 5, columnspan = 2)
            ...
            self.Button_DELETE_m3u = tk.Button(self.Frame10, text = "DEL .m3u",width = 7, height = 1,command = self.DelPL_m3u)
        ...
        if self.cutdown == 5: # Pi 7" Display 800 x 480 (Simple layout)
            ...
            self.B13.grid(row = 5, column = 4, pady = 0)
            self.B11 = tk.Button(self.Frame10, text = "RELOAD", font = 50,width = 10, height = 3, bg = "#c5c",command = self.RELOAD_List, wraplength=80, justify=CENTER)
            self.B11.grid(row = 6, column = 0)
            B12 = tk.Button(self.Frame10, text = "QUIT",bg = "light blue",  width = 10, height = 3,command=self.exit).grid(row = 6, column = 3)
    ...
    def Play(self):
        if (self.HP4_backlight == 1 or self.WS28_backlight == 1)and (self.album_start == 1 or self.stopstart == 1):
            self.gpio.set_PWM_dutycycle(self.pwm_pin,self.bright)
        ...
        if len(alsaaudio.mixers()) > 0:
            for mixername in alsaaudio.mixers():
                if str(mixername) == "PCM" or str(mixername) == "Master" or str(mixername) == "Capture" or str(mixername) == "Headphone" or str(mixername) == "HDMI":
                    ...
                    self.Button_Pause.config(fg = "light gray",bg = "light gray")
            self.m.setvolume(self.volume)
        if self.album_start == 0 and self.stopstart != 1:
            self.stopstart = 1
            self.light_on = time.time()
            self.play = 0
    ...
    def nextAZ(self):
        if self.album_start == 0 and self.stopstart == 0: #and os.path.exists(self.que_dir):
            #if self.cutdown != 1  and self.cutdown != 5:
            #    self.Disp_Name_m3u.config(background="light gray", foreground="black")
            #if self.play == 2:
            #    self.play = 0
    ...
    def RELOAD_List(self):
        if (self.WS28_backlight == 1 or self.HP4_backlight == 1) :
                self.gpio.set_PWM_dutycycle(self.pwm_pin,self.bright)
                self.light_on = time.time()
        if self.paused == 0 and self.album_start == 0 and self.stopstart == 0:
            if self.cutdown == 0 or self.cutdown == 2 or self.cutdown == 3:
                self.Disp_Name_m3u.config(background="light gray", foreground="black")
            ...
            self.sorted == 0
            if self.cutdown == 0 or self.cutdown == 2 or self.cutdown == 3:
                self.Button_AZ_artists.config(bg = "light blue",fg = "black",text = "A-Z Artists")
...
def main():
    ...
    elif cutdown == 4 and root.winfo_screenwidth() == 480 and root.winfo_screenheight() == 320:
        root.geometry("480x320")
    elif cutdown == 4:
