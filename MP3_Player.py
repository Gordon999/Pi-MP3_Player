...
class MP3Player(Frame):
    ...
    def initUI(self):
        ...
        # check Lasttrack3.txt exists, if not then write default values. Used for recalling last Radio Station, and restarting if using a Pi Zero.
        track  = 0
        radio  = 0
        volume = 80
    ...
    def Play(self):
        ...
        self.f_volume = self.volume
        if self.cutdown != 4 and self.cutdown != 5 and self.cutdown != 1 and self.Radio_ON == 0:
    ...
    def Play_Album(self):
        ...
        self.light_on = time.time()
        self.f_volume = self.volume
        if self.cutdown != 4 and self.cutdown != 5 and self.Radio_ON == 0:
            self.L6.config(text= "Playlist :")
        if self.cutdown == 0 or self.cutdown == 2:
            self.Button_volume.config(text = self.volume)
        else:
        ...
        if self.paused == 0 and len(self.tunes) > 0 and self.album_start == 0 and self.stopstart == 0 and self.Radio_ON == 0:
            ...
            self.repeat_track = 0
            if self.trace == 1:
                print ("Play Album1",self.track_no)
            if len(self.tunes) > 2:
                self.shuffle_on = 0
                if self.cutdown == 0 or self.cutdown == 2 or self.cutdown == 3:
                    self.Disp_Name_m3u.config(background="light gray", foreground="black")
                ...
                while stop == 0:
                    ...
                    if self.trace == 1:
                        print ("Play Album1x",counter,self.tunes[counter])
                        print (self.artist_name,self.artist_name2,self.album_name[0:-1],self.album_name2)
    ...
    def FAV_List(self):
        if os.path.exists(self.que_dir) and self.Radio_ON == 0:
            ...
            self.Disp_Name_m3u.config(background="light gray", foreground="black")
            self.Disp_Name_m3u.delete('1.0','20.0')
            print (self.drive_name1,self.drive_name2,self.drive_name,self.artist_name,self.album_name,self.track_name)
            with open(self.m3u_dir + Name + ".m3u", 'a') as f:
                f.write("/" + self.drive_name1 + "/" + self.drive_name2 + "/" + self.drive_name + "/" + self.artist_name + "/" + self.album_name + "/" + self.track_name + "\n" )
            self.m3us = glob.glob(self.m3u_dir + "*.m3u")
            self.m3us.remove(self.m3u_dir + self.m3u_def + ".m3u")
    ...
    def isConnected(self):
        try:
            ...
            if self.auto_radio == 0 and self.auto_record == 0:
                messagebox.showinfo("WARNING!","No Internet found!")
        return False
    ...
    def RadioX(self):
        ...
        if self.Radio_ON == 1 and self.Radio_RON == 1 and self.record == 1:
            ...
            if self.cutdown != 5:
                ...
                if self.cutdown != 1:
                    self.Button_Track_m3u.config(bg  = "light green", fg = "black")
                    self.Button_Album_m3u.config(bg  = "light green", fg = "black", text = "ADD Album to .m3u")
                    self.Button_Artist_m3u.config(bg = "light green", fg = "black", text = "ADD Artist to .m3u")
                self.Button_Start.config(bg = "green",fg = "white",text = "PLAY Playlist")
                self.Disp_plist_name.config(text=" " + self.que_dir[len(self.m3u_dir):])
            else:
                self.Button_Next_Artist.config(text="Artist >")
            ...
            if out == True:
                ...
                if self.Radio_Stns[self.Radio + 2] == 1 and self.record == 1:
                    ...
                    if self.cutdown != 1 and self.cutdown != 4 and self.cutdown != 5 and self.cutdown != 3:
                        self.Button_Pause.config(fg = "black", bg = "light blue", text = "RECORD")
                        self.L8.config(text = ".mp3")
                        self.L6.config(text="")
                else:
    ...
    def Check_Record(self):
        ...
        if self.Radio_RON == 1 and ((time.time() - self.rec_begin > self.record_time_min)  or freeram < self.ram_min) and self.record == 1 and self.oldtrack != self.track_nameX[self.counter]:
            if self.trace == 1:
            ...
            if self.cutdown != 1:
                ...
                self.Button_Radio.config(bg = "orange",fg = "black", text = "STOP")
            self.Copy_Record()
        if self.track_nameX[self.counter] != " - .mp3": 
